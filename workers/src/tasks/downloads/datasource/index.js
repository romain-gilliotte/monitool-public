...
async function generateForm(io, id, start, end, sites, dataSource, language, orientation, format) {
    ...
    // Generate form
    let form = null;
    if (format === 'pdf') form = new PaperForm(ql, orientation, language);
    else if (format === 'xlsx') form = new ExcelForm(ql);
    else throw new Error('Unsupported format');

    const content = await form.generateOutput();
