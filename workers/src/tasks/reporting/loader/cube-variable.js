 ...
 */
async function getVariableCube(io, project, upto, variableId, aggregate, dices) {
    ...
    const [preAggregatedCube, postAggregatedCube] = await Promise.all([
        getPreAggregated(io, upto, dimensions, project._id, variable.id, rules, aggregate, dices),
        getPostAggregated(io, upto, dimensions, project._id, variable.id, rules, aggregate, dices),
    ]);
 ...
 */
async function getPostAggregated(
    io,
    upto,
    dimensions,
    projectId,
    ...
    await iterateCubes(io, upto, projectId, variableId, rules, inputCube => {
 ...
 */
async function getPreAggregated(
    io,
    upto,
    dimensions,
    projectId,
    ...
    cube.createStoredMeasure('main', rules);
    await iterateCubes(io, upto, projectId, variableId, rules, cube.hydrateFromCube.bind(cube));
