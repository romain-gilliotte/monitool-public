...
router.post('/project/:id/input', validateBody('input'), async ctx => {
	...
	if (!project)
		allowed = false;
	else if (project.owner === ctx.state.profile.email)
		allowed = true;
	else {
		...
		allowed = invitation && ctx.request.body.content.every(content => {
			const dataSource = project.forms.find(ds => ds.elements.some(v => v.id == content.variableId));
