...
module.exports = project => {
    ...
    if (project.forms.some(ds => ds.elements.every(variable => !variable.active)))
        errors.push({
            code: 'all vars inactive',
            message: 'At least one variable must be active by form',
        });

    const peInactive = project.forms.some(ds =>
        ds.elements.some(variable =>
            variable.partitions.some(p => p.elements.every(pe => !pe.active))
        )
    );
    if (peInactive)
        errors.push({
            code: 'all partitions elements inactive',
            message: 'At least one pe must be active by partition',
        });

    if (!variables.every(v => v.distribution >= 0 && v.distribution <= v.partitions.length))
        errors.push({
            code: 'invalid distribution',
            message: 'must be between 0 and partition.length included',
        });
    ...
    const isValid =
        ...
        project.groups.every(g =>
            g.members.every(siteId => project.entities.find(s => s.id === siteId))
        ) &&
        project.forms.every(f =>
            f.entities.every(siteId => project.entities.find(s => s.id === siteId))
        );

    if (!isValid) errors.push({ code: 'references', message: 'no dead references' });

    // Check indicators
    const indicators = getIndicators(project);
    const indicatorErrors = indicators.reduce(
        (m, i) => [...m, ...checkIndicator(i, variables)],
        []
    );
...
function checkIndicator(indicator, variables) {
    if (!indicator.computation) return [];
            ...
            for (let partitionId in parameter.filters) {
                ...
                if (partition) {
                    if (!filter.every(peId => partition.elements.find(pe => pe.id == peId)))
                        errors.push({
                            code: 'references',
                            message: 'dead reference from indicator to pelement',
                        });
                } else
                    errors.push({
                        code: 'references',
                        message: 'dead reference from indicator to partition',
                    });
            }
        } else
            errors.push({
                code: 'references',
                message: 'dead reference from indicator to variable',
            });
    }
