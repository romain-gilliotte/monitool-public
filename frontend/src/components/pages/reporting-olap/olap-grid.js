    ...
    controller: class {
        ...
        _buildHeader(colss) {
            ...
            for (var i = 0; i < colss.length; ++i) {
                ...
                for (var k = 0; k < numCols; ++k) row.cols.push(colss[i][k % colss[i].length]);
        ...
        _buildDataRows(rowss, colss, data) {
            ...
            product(rowss).forEach((headers, rowIndex) => {
                ...
                const dataCols = colsPaths.map(els => {
                    ...
                    for (var i = 0; i < numEls; ++i) result = result[els[i].id];
        ...
        _addFakeDimensionToData(data, at) {
            if (at == 0) return { all: data };
            else {
                const obj = {};
                for (let key in data) obj[key] = this._addFakeDimensionToData(data[key], at - 1);
                return obj;
            }
        }
    },
