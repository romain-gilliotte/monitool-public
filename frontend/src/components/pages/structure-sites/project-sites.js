    ...
    controller: class {
        ...
        onFieldChange() {
            this.onProjectUpdate({
                ...
                isValid:
                    this.editableProject.entities.every(s => s.name.length > 0) &&
                    this.editableProject.groups.every(
                        g => g.name.length > 0 && g.members.length > 0
                    ),
            });
        }

        onCreateSiteClicked() {
            this.editableProject.entities.push({
                id: uuid(),
                name: '',
                active: true,
            });
        ...
        onDeleteSiteClicked(site) {
            ...
            if (window.confirm(question)) {
                this.editableProject.entities = this.editableProject.entities.filter(
                    s => s !== site
                );
        ...
        onCreateGroupClicked() {
            this.editableProject.groups.push({
                id: uuid(),
                name: '',
                members: [],
            });
        ...
        onDeleteGroupClicked(groupId) {
            this.editableProject.groups = this.editableProject.groups.filter(
