...
module.component(__componentName, {
    ...
    controller: class {
        $onChanges(changes) {
            ...
            this.editablePartitions.forEach(
                p => (this.numActiveElements[p.id] = p.elements.filter(e => e.active).length)
            );

            this.availablePartitions = this.editablePartitions.filter(p => p.active);

            // Tell the template about the table layout
            const indexes = this.partitions
            ...
            const width = this.table.headerRows.reduce(
                (m, i) => m * this.numActiveElements[this.partitions[i].id],
                1
            );
            const height = this.table.leftCols.reduce(
                (m, i) => m * this.numActiveElements[this.partitions[i].id],
                1
            );
        ...
        onValueChange(index) {
            const missing = this.partitions.find(
                p => !this.editablePartitions.find(op => op.id === p.id)
            );
            const dupIndex = this.editablePartitions.findIndex(
                (op, i) => i !== index && op.id === this.editablePartitions[index].id
            );
