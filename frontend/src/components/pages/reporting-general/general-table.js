...
import {
    buildQueryFromIndicator,
    buildQueryFromVariable,
    getQueryDimensions,
...
module.component(__componentName, {
    ...
    controller: class {
        ...
        _translateHeaders() {
            ...
            const leftHeaders = target.querySelectorAll(
                '.section-header-row,.row-graph,.row-title,.row-dimension'
            );
        ...
        onRowPlotClicked(rowId) {
            if (!this.plots[rowId]) this.plots[rowId] = { active: false, data: null };
        ...
        onRowPlotData(rowId, data) {
            ...
            if (this.plots[rowId].active) this._updatePlots();
        }
        ...
        _makeColumnsFromQuery(query) {
            if (query.aggregate.length !== 1) throw new Error('Not supported');
        ...
        _updateRows() {
            const newRows = [
                ...this.project.logicalFrames.reduce(
                    (m, logFrame) => [...m, ...this._makeRowsFromLogicalFramework(logFrame)],
                    []
                ),
                ...this._makeRowsFromExtraIndicators(this.project.extraIndicators),
                ...this.project.forms.reduce(
                    (m, dataSource) => [...m, ...this._makeRowsFromDataSource(dataSource)],
                    []
                ),
            ];
            ...
            return [
                ...
                ...(this.sectionOpen[rowId]
                    ? [
                          ...
                          ...logFrame.indicators.reduce(
                              (m, indicator, i) => [
                                  ...m,
                                  ...this._makeRowsFromIndicator(
                                      `${rowId}.i.${i}`,
                                      logFrame,
                                      indicator
                                  ),
                              ...
                              (m, purpose, pIndex) => [
                                  ...
                                  ...purpose.outputs.reduce(
                                      (m, output, oIndex) => [
                                          ...
                                          ...output.indicators.reduce(
                                              (m, indicator, i) => [
                                                  ...m,
                                                  ...this._makeRowsFromIndicator(
                                                      `${rowId}.p.${pIndex}.o.${oIndex}.i.${i}`,
                                                      logFrame,
                                                      indicator,
                                                      2
                                                  ),
                                          ...
                                          ...output.activities.reduce(
                                              (m, activity, aIndex) => [
                                                  ...
                                                  ...activity.indicators.reduce(
                                                      (m, indicator, i) => [
                                                          ...m,
                                                          ...this._makeRowsFromIndicator(
                                                              `${rowId}.p.${pIndex}.o.${oIndex}.a.${aIndex}.i.${i}`,
                                                              logFrame,
                                                              indicator,
                                                              3
                                                          ),
        ...
        _makeRowsFromExtraIndicators(extraIndicators) {
            if (extraIndicators.length === 0) return [];
        ...
        _makeRowsFromQuery(
            ...
            if (disagregateBy.id === 'computation') {
                for (let paramName in query.parameters) {
                    ...
                    const variableName = this.project.forms
                        ...
                        .find(v => v.id == parameter.variableId).name;
            ...
            // Generic split
            const dimension = getQueryDimensions(this.project, query).find(
                d => d.id == disagregateBy.id
            );
            if (!dimension) {
            ...
            const attributes = disagregateBy.attribute
                ? [disagregateBy.attribute]
