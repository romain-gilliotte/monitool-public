...
module.directive(__componentName, () => ({
    ...
    controller: class {
        ...
        _getText(value, complete, raw) {
            if (value === null) {
                if (complete) return '<i class="fa fa-exclamation"></i>';
                else return '<i class="fa fa-question-circle-o"></i>';
                ...
                if (this.unit) text = text + this.unit;
        ...
        _getTitle(value, complete, raw) {
            if (value === null) {
                if (complete) return 'Division by zero';
                else return 'This value cannot be computed because the data entry was not done.';
            } else if (typeof value === 'number') {
                if (!raw && !complete)
                    return 'This value was computed from data which is interpolated and incomplete.';
                else if (!raw) return 'This value was computed from data which is interpolated.';
                else if (!complete) return 'This value was computed from data which is incomplete.';
            }
        }
    },
}));
