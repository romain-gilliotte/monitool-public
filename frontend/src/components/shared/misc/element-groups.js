...
module.component(__componentName, {
    ...
    controller: class {
        ...
        _model2view(model, elements, groups) {
            ...
            if (model.length == elements.length) return [{ name: 'project.all_elements' }];
            else if (model.length == 0) return [{ name: 'shared.none' }];
            ...
            return [...selectedGroups, ...additionalIds.map(id => elements.find(e => e.id == id))];
        }
    },
});
