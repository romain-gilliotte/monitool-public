...
export default {
	...
	project: {
		...
		history: {
			...
			forms_entities_replace: "Remplace le lieu <code>{{beforeItem.name}}</code> par <code>{{afterItem.name}}</code> dans la source de données <code>{{form.name}}</code>",
			...
			forms_elements_replace: "Remplace la variable <code>{{before.name}}</code> par <code>{{after.name}}</code> dans <code>{{form.name}}</code>",
			...
			forms_elements_partitions_elements_active_replace: "{{after ? 'Active' : 'Désactive'}} la saisie de l'élément <code>{{element.name}}</code> de la désagrégation <code>{{partition.name}}</code> de la variable <code>{{variable.name}}</code>",
		...
		saving_failed: "Impossible de sauvegarder vos modifications, probablement car vous n'êtes plus connecté à internet. Gardez cette fenêtre ouverte, et retentez de sauvegarder quand vous serez connecté à internet.",
