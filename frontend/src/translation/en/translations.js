export default {
    shared: {
        ...
        sure_to_leave:
            'You made changes. Click OK to confirm that you want to leave without saving.',
    },
    menu: {
        language: 'Language',
        french: 'French',
    ...
    project: {
        ...
        history: {
            ...
            name_replace:
                'Rename the project from <code>{{before}}</code> to <code>{{after}}</code>',
            start_replace:
            ...
            country_replace:
                'Change the project country from <code>{{before}}</code> to <code>{{after}}</code>',
            ...
            entities_name_replace:
                'Rename the site <code>{{before}}</code> to <code>{{after}}</code>',
            entities_active_replace:
                "{{after ? 'Enable' : 'Disable'}} data entry on site <code>{{entity.name}}</code>",
            ...
            groups_name_replace:
                'Rename the group <code>{{before}}</code> to <code>{{after}}</code>',
            groups_members_add:
                'Add the site <code>{{item.name}}</code> to the group <code>{{group.name}}</code>',
            groups_members_move: 'Reorder the sites of the group <code>{{group.name}}</code>',
            groups_members_remove:
                'Remove the site <code>{{item.name}}</code> from the group <code>{{group.name}}</code>',

            forms_add: 'Create the data source <code>{{item.name}}</code>',
            forms_move: 'Reorder the data sources of the project',
            forms_replace:
                'Replace data sources <code>{{before.name}}</code> by <code>{{after.name}}</code>',
            forms_remove: 'Delete the data source <code>{{item.name}}</code>',
            forms_name_replace:
                'Rename the data source <code>{{before}}</code> to <code>{{after}}</code>',
            forms_periodicity_replace:
                'Change the periodicity of the data source <code>{{form.name}}</code> from <code>{{before}}</code> to <code>{{after}}</code>',
            forms_active_replace:
                "{{after ? 'Enable' : 'Disable'}} data entry on data source <code>{{form.name}}</code>",

            forms_entities_add:
                'Add the site <code>{{item.name}}</code> to the data source <code>{{form.name}}</code>',
            forms_entities_move: 'Reorder the sites of the data source <code>{{form.name}}</code>',
            forms_entities_remove:
                'Remove the site <code>{{item.name}}</code> from the data source <code>{{form.name}}</code>',
            forms_entities_replace:
                'Replace the site <code>{{beforeItem.name}}</code> by <code>{{afterItem.name}}</code> in data source <code>{{form.name}}</code>',

            forms_elements_add:
                'Add the variable <code>{{item.name}}</code> to <code>{{form.name}}</code>',
            forms_elements_move:
                'Reorder the variables of the data source <code>{{form.name}}</code>',
            forms_elements_remove:
                'Remove the variable <code>{{item.name}}</code> from <code>{{form.name}}</code>',
            forms_elements_replace:
                'Replace variable <code>{{before.name}}</code> by <code>{{after.name}}</code> in <code>{{form.name}}</code>',

            forms_elements_name_replace:
                'Rename the variable <code>{{before}}</code> to <code>{{after}}</code>',
            forms_elements_active_replace:
                "{{after ? 'Enable' : 'Disable'}} data entry on variable <code>{{variable.name}}</code>",
            forms_elements_geoAgg_replace:
                'Change the aggregation rule (location) of <code>{{variable.name}}</code> from <code>{{before}}</code> to <code>{{after}}</code>',
            forms_elements_timeAgg_replace:
                'Change the aggregation rule (time) of <code>{{variable.name}}</code> from <code>{{before}}</code> to <code>{{after}}</code>',
            forms_elements_distribution_replace:
                'Change the format of the input table of <code>{{variable.name}}</code>',

            forms_elements_partitions_add:
                'Create the disaggregation <code>{{item.name}}</code> in <code>{{variable.name}}</code>',
            forms_elements_partitions_move:
                'Reorder the disaggregations of <code>{{variable.name}}</code>',
            forms_elements_partitions_remove:
                'Delete the disaggregation <code>{{item.name}}</code> from <code>{{variable.name}}</code>',
            forms_elements_partitions_name_replace:
                'Rename the disaggregation <code>{{before}}</code> to <code>{{after}}</code> in variable <code>{{variable.name}}</code>',
            forms_elements_partitions_active_replace:
                "{{after ? 'Enable' : 'Disable'}} data entry on disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>",
            forms_elements_partitions_aggregation_replace:
                'Change aggregation mode from <code>{{before}}</code> to <code>{{after}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',

            forms_elements_partitions_elements_add:
                'Add element <code>{{item.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_elements_move:
                'Reorder elements of disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_elements_remove:
                'Delete element <code>{{item.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_elements_name_replace:
                'Rename element <code>{{before}}</code> to <code>{{after}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_elements_active_replace:
                "{{after ? 'Enable' : 'Disable'}} data entry on disaggregation element <code>{{element.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>",

            forms_elements_partitions_groups_add:
                'Add group <code>{{item.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_move:
                'Reorder groups in <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_remove:
                'Delete group <code>{{item.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_name_replace:
                'Rename group <code>{{before}}</code> to <code>{{after}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_members_add:
                'Add <code>{{item.name}}</code> to group <code>{{group.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_members_move:
                'Reorder group members of <code>{{group.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            forms_elements_partitions_groups_members_remove:
                'Remove <code>{{item.name}}</code> from group <code>{{group.name}}</code> in disaggregation <code>{{partition.name}}</code> of variable <code>{{variable.name}}</code>',
            ...
            logicalFrames_entities_add:
                'Add the site <code>{{item.name}}</code> to the logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_entities_move:
                'Reorder the sites of the logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_entities_remove:
                'Remove the site <code>{{item.name}}</code> from the logical framework <code>{{logicalFrame.name}}</code>',

            logicalFrames_name_replace:
                'Rename logical framework <code>{{before}}</code> to <code>{{after}}</code>',
            logicalFrames_goal_replace:
                'Change general objective <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_start_replace:
                'Change start date <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_end_replace:
                'Change end date <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            ...
            logicalFrames_purposes_outputs_move:
                'Reorder results in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_remove:
                'Remove result <code>{{item.description}}</code> from logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_description_replace:
                'Change result description <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_assumptions_replace:
                'Change result assumptions <code>{{output.description}}</code> from <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',

            logicalFrames_purposes_outputs_activities_add:
                'Add activity <code>{{item.description}}</code> to logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_activities_move:
                'Reorder activities in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_activities_remove:
                'Remove activity <code>{{item.description}}</code> from logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_purposes_outputs_activities_description_replace:
                'Change activity description <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',

            logicalFrames_indicators_add:
                'Add indicator <code>{{item.display}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_indicators_move:
                'Move indicator <code>{{item.display}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            ...
            logicalFrames_indicators_display_replace:
                'Rename indicator <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_indicators_colorize_replace:
                'Change indicator <code>{{indicator.display}}</code> colorization from <code>{{before}}</code> to <code>{{after}}</code> in logical framework <code>{{logicalFrame.name}}</code>',
            logicalFrames_indicators_computation_replace:
                'Change indicator <code>{{indicator.display}}</code> computation in logical framework <code>{{logicalFrame.name}}</code>',
            ...
            extraIndicators_target_replace:
                'Change target of extra indicator <code>{{extraIndicator.display}}</code> from <code>{{before}}</code> to <code>{{after}}</code>',
            extraIndicators_display_replace:
                'Rename extra indicator <code>{{before}}</code> to <code>{{after}}</code>',
            extraIndicators_colorize_replace:
                'Change extra indicator <code>{{extraIndicator.display}}</code> colorization from <code>{{before}}</code> to <code>{{after}}</code>',
            extraIndicators_computation_replace:
                'Change extra indicator <code>{{extraIndicator.display}}</code> computation',
        },
        ...
        partition_help_name:
            'This name will appear on multiple reporting tables. It names the disaggregation that you want to create on your variable',
        partition_help_elements:
            'Elements from the disaggregation must be mutually exclusive, and it should be possible to find the total value by aggregating them.',
        partition_help_aggregation:
            'How to find the total value by aggregating the elements described above?',
        ...
        saving_failed:
            'Unable to save the changes, probably because of connectivity issues. Keep this window open, and try saving again once you are connected to the internet.',

        partition_general: 'General',
        partition_general_placeholder:
            'ex: Age group, gender, motive for consultation, pathology, referral status, ...',
        partition_elements: 'Elements',
        aggregation_lab: 'How to group elements together?',
        partition_name: 'Name',
        partition_name_placeholder:
            'ex: Less than 12 years old, male, social consultation, flu, community referral, ...',
        ...
        no_variable:
            'No variable is defined on this data source. Click "Add a variable" to create one!',
        ...
        are_you_sure_to_uninvite:
            'Are you sure that you want to remove this project from your list? The owner will have to invite you again if you need access later. Confirm to remove.',
        ...
        intervention_logic_goal_ph:
            'e.g. Reduce mortality and morbidity of the populations affected by the crisis',
        intervention_logic_purpose_ph:
            'e.g. Improve access to health care for population affected by the crisis in the districts of Bimbo and Begoua',
        ...
        logframe_edit_help_end:
            'Date until which indicators will be computed for this logical framework',

        logframe_help_sites:
            'Among sites identified in "Collection sites", which one are relevant for this logical framework?',
        logframe_help_name:
            'Name this logical framework to be able to identify it easily. For instance, with the name of the relevant donor',
        logframe_help_goal: "Describe the project's contribution on a program or policy",
        logframe_help_goal_indicators:
            'Enter here the indicators that allow to measure the general objective',
        logframe_help_purpose_desc:
            'Describe the tangible advantages that are provided to the beneficiaries',
        logframe_help_purpose_assumptions:
            'External factors that could jeopardize reaching the specific objective',
        logframe_help_purpose_indicators:
            'Enter here the indicators that allow to measure the specific objective',
        logframe_help_output_desc: 'Product or tangible service brought by the project',
        logframe_help_output_assumptions:
            'External factors that could jeopardize reaching the result',
        logframe_help_output_indicators:
            'Enter here the indicators that allow to measure the result',
        ...
        basics_help_end:
            'The end date is the moment when your project closes its data collection. If unknown, enter a date far into the future.',

        collection_edit_help_name:
            'What is the name of the data source that you want to extract data from? i.e. "Electronic medical record", "Health center tally sheet", "NHIS report", ...',
        ...
        collection_edit_help_geoagg:
            'In a project with two sites, if <code>{{name}}</code> is 10 for the first and 20 for the second, what is the value for the complete project?',
        ...
        collection_edit_help_order:
            'How do you wish to show the disaggregations on the input form?',
        ...
        indicator_help_description:
            'Context of the data collection, details on how to compute the indicator...',
        indicator_help_display:
            'Name your indicator. The name should come from a catalog to be consistent with other projects.',
        indicator_help_baseline:
            'What was the value of the indicator before the first activities? Tick the checkbox to specify.',
        indicator_help_target:
            'What is the target for this indicator? Tick the checkbox to specify.',
        indicator_help_colorize:
            'Do you wish to have colors (red, orange, green) on reporting for this indicator?',
        indicator_help_computation:
            'How to compute this indicator from the variables that you collected in data sources?',
