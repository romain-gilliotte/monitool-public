...
export default class Project {
    ...
    _sanitizeIndicator(indicator) {
        if (indicator.computation === null) return;
        ...
        for (var key in indicator.computation.parameters) {
            ...
            this.forms.forEach(f => {
                f.elements.forEach(e => {
                    if (e.id === parameter.elementId) element = e;
                });
            });
    ...
    async save() {
        ...
        let response;
        if (this._id) response = await axios.put('/project/' + this._id, payload);
        else response = await axios.post('/project', payload);
